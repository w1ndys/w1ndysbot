...
[Git 提交和 PR 工作流规则]
- Date: 2026-05-15
- Context: 用户要求后续提交和 PR 规则按指定格式执行
- Instructions:
  - 根据当前对话信息或 git 状态生成 git commit 命令并执行，使用中文环境显示信息。
  - commit message 使用专业格式：`改动类型(改动文件): 改动内容`，例如 `feat(main.py): 优化了函数结构`。
  - 提交过程需要具备良好的异常处理，将报错信息显式返回给用户。
  - 所有提交信息使用一行 `git commit` 命令；多行描述信息合并成一行命令一次性提交，使用 `-m` 参数拼接。
  - 提交信息里的贡献者 name 使用 `W1ndys`，邮箱使用 `w1ndys@qq.com`。
  - 根据改动分类分成多次 commit 提交。
  - 提交前一次性阅读所有改动。
